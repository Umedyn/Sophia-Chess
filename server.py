...
def submit_move(match, token, turn_id, slot) -> str:
    ...
    with match.lock:
        ...
        match.active ^= 1                  # swap 0<->1; the machine is side-agnostic
        return "accepted"
...
def _begin(match):
    ...
    with match.lock:
        ...
        for i, p in enumerate(match.players):
            p.slot  = i
            p.label = "white" if i == 0 else "black"
...
def _broadcast_terminal(match):
    ...
    with match.lock:
        ...
        for p in ai_players:
            turn_id = _next_turn_id(match)
            outbound.append(
                (p, _build_payload(match, turn_id, terminal=True, perspective=p.label))
            )
...
if __name__ == "__main__":
    ...
    start_match(ChessGame, "Chess", [p1, p2])   # ChessGame() called fresh each game
